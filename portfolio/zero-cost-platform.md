...
in-browser editor — reachable through a Konami-code terminal — that authenticates with a
GitHub personal access token, commits directly through the GitHub Contents API, and (in the
default single-repository setup) retriggers the site's own redeploy on that commit. All of it
served on GitHub Pages, at no hosting cost.
